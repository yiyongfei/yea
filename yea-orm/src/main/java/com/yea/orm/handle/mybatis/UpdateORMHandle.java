...
public class UpdateORMHandle<T> extends AbstractORMHandle<T> {
    
	public UpdateORMHandle() {
		super(ORMConstants.ORM_LEVEL.M_UPDATE.getCode());
	}

	@Cache(AnnotationConstants.CommandType.SET)
	@Override
	protected T execute(SqlSessionTemplate sessionTemplate, ORMParams dto) throws Exception {
		return (T) Integer.valueOf(sessionTemplate.update(dto.getSqlid(), dto.getParam()));
	}

	@Override
	public void setNextHandle() {
		// TODO Auto-generated method stub
		ORMHandle<T> nextHandle = ORMHandleFactory.getInstance(ORMConstants.ORM_LEVEL.M_LOAD);
