...
import com.yea.core.loadbalancer.Pair;
...
public class LoadBalancerContext implements IClientConfigAware {
    ...
    protected IRetryHandler defaultRetryHandler = new DefaultLoadBalancerRetryHandler();
    ...
    public LoadBalancerContext(ILoadBalancer lb, IClientConfig clientConfig, IRetryHandler handler) {
    ...
    /**
     * Use {@link #setRetryHandler(IRetryHandler)} 
     */
    @Deprecated
     ...
     */
    public void noteRequestCompletion(ServerStats stats, Object response, Throwable e, long responseTime, IRetryHandler errorHandler) {
    	if (stats == null) {
    		return;
    	}
        try {
            recordStats(stats, responseTime);
            IRetryHandler callErrorHandler = errorHandler == null ? getRetryHandler() : errorHandler;
            if (callErrorHandler != null && response != null) {
                ...
                if (callErrorHandler.isCircuitTrippingException(e)) {
                	/*如果抛出的异常是SocketException或SocketTimeoutException，则记录连接失败次数*/
     ...
     * {@link #reconstructURIWithServer(BalancingNode, java.net.URI)} methods to get the complete executable URI.
     */
     ...
     */
    public BalancingNode getServerFromLoadBalancer(URI original, Object loadBalancerKey) throws ClientException {
        String host = null;
        int port = -1;
        if (original != null) {
        ...
        if (host == null) {
            ...
            if (lb != null){
                BalancingNode svc = lb.chooseNode(loadBalancerKey);
                if (svc == null){
        ...
        return new BalancingNode(host, port);
    }

    public URI reconstructURIWithServer(BalancingNode server, URI original) {
    ...
    public final ServerStats getServerStats(BalancingNode server) {
    ...
    public boolean handleSameServerRetry(BalancingNode server, int currentRetryCount, int maxRetries, Throwable e) {
    ...
    public final void setRetryHandler(IRetryHandler retryHandler) {
