 ...
 * {@link BalancingNode#getZone()}) will be filtered out. By default, zone affinity and
 * exclusivity are turned off and nothing is filtered out.
 * 
 * @author stonse
 *
 */
public class ZoneAffinityServerListFilter<T extends BalancingNode> extends AbstractNodeListFilter<T>
	...
	private boolean shouldEnableZoneAffinity(Collection<T> filtered) {
	...
	@Override
	public Collection<T> getFilteredListOfNodes(Collection<T> servers) {
		if (zone != null && (zoneAffinity || zoneExclusive) && servers != null && servers.size() > 0) {
			Collection<T> filteredServers = Lists
