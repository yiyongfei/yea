 ...
 * 
 */
public class HeartBeatServerHandler extends AbstractHeartBeatServerHandler {
     ...
     */
    public HeartBeatServerHandler(int readerIdleTimeSeconds, int writerIdleTimeSeconds) {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (message.getHeader() != null && message.getHeader().getType() == RemoteConstants.MessageType.HEARTBEAT_RESP.value()) {
        	LOGGER.info("服务端" + ctx.channel().localAddress() + "从" + ctx.channel().remoteAddress() + "接收"+message.getHeader().getAttachment().get(EVT_STATE)+"心跳检测响应！");
